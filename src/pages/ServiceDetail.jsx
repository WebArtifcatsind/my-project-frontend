...
const serviceKey = title.replace(/\s+/g, "_"); // <-- ADD THIS
  ...
  const getBPP = () => {
    ...
    // phones stay 1 per page
    if (w <= 767) return 1;

    // allow tolerance around exact buckets (dpi/scrollbars)
    const tol = 36;
    const near = (x, y) => Math.abs(x - y) <= tol;

    const isSpecialBucket =
      (near(w, 1820) && near(h, 1180)) ||
      (near(w, 1834) && near(h, 1194)) ||
      (near(w, 1280) && near(h, 800)) ||
      (near(w, 800) && near(h, 1280)) ||
      (near(w, 1180) && near(h, 1820)) ||
      (near(w, 1194) && near(h, 1834));

    // tablets (≤1024) OR your special buckets -> 2 per page
    if ((w <= 1024 && w > 767) || isSpecialBucket) return 2;

    // desktop default -> 4 per page
    return 4;
  };
  ...
  const goPrev = () => setCurrentPage((p) => (p > 0 ? p - 1 : 0));
  ...
  // style only when we are in 2-per-page mode (tablets & special buckets)
  const tabletTwoPerStyle =
    benefitsPerPage === 2 ? { flex: "0 0 calc(50% - 2rem)" } : undefined;
    ...
    <div className="service-detail-page" id="service-detail-top">
      <section className="service-hero" aria-labelledby="service-title">
        <div className="particles-container">
          <Canvas camera={{ position: [0, 0, 10], fov: 75 }} dpr={[1, 2]} gl={{ antialias: true }}>
        ...
        <div className="hero-content-split">
          <div className="hero-left">
            <h1
              id="service-title"
              className={`service-title ${splitTitle(title).length === 1 ? "single-line" : ""}`}
            >
          ...
          <div className="why-choose-content">
            <div className="why-choose-text">
              <div
                ...
                style={{ opacity: 0, transform: "translateX(50px)", color: "black" }}
              >
                {service.whyChooseUs}
                <p className="additional-paragraph">
                  With our proven track record in delivering enterprise-grade IT solutions, we bring
                  deep technical expertise and business acumen to every project. Our team stays
                  ahead of technology trends to provide future-proof solutions tailored to your
                  specific needs.
                </p>
              </div>
            </div>
          </div>
          <div className="why-choose-image" style={{ opacity: 0, transform: "translateX(-100px)" }}>
            <img
