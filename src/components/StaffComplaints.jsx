...
const StaffComplaints = ({ token }) => {
  ...
  const fetchAssignedComplaints = async () => {
    try {
      const res = await axios.get(
        "https://my-project-backend-tan.vercel.app/api/client/complaints/assigned",
  ...
  const markComplaintResolved = async (id) => {
    try {
      await axios.put(
        `https://my-project-backend-tan.vercel.app/api/client/complaint/resolve/${id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteStaffComplaint = async (id) => {
    ...
    try {
      await axios.delete(
        `https://my-project-backend-tan.vercel.app/api/client/complaints/staff/${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div className="complaints-grid">
          {assignedComplaints.map((comp) => (
            <div key={comp.id} className="complaint-card">
              ...
              {comp.file && (
                <a
                  href={`https://my-project-backend-tan.vercel.app/uploads/complaints/${comp.file}`}
                  download
